...
| 类型 | 有状态组件   | 无状态组件                                   |
| ---- | ------------ | -------------------------------------------- |
| 权力 | 有权改变状态 | 无权改变状态(404界面)，只能接收界面。-子组件 |
...
```js
react有自己的跨平台兼容性，为了自己的时间系统模式，React 模拟原生 DOM 事件所有能力的一个事件对象，被通常叫做react合成事件。
...
### 9.0.9  data | useState |  useRef

```
useState 这个值的改变不能够同步更新（他的弊端就是不能在更新后立刻拿到改变后的值，state的问题在于一旦修改了它就会造成组件的重新渲染。）
...
--3.import styles from "./job.module.css'
className={styles.container}
```



### 9.0.12 ErrorBoundary
...
react16 新增了两个生命周期 componentdidcatch 和 static getDerivedStateFromError . 从框架级别让我们更方便捕捉异常并显示备用 ui。
...
### 9.0.13 cloneElement | createElement
...
### 9.1.18 react事件处理逻辑，调用setstate的时候，发生了啥





### 9.1.19 函数组件和类组件本质的区别。为什么先执行setstate然后进行render





### 9.1.20 render props和高阶组件的区别
...
### 9.1.22 react为什么要将setState其设计成异步？为什么 SetTimeout 或者原生事件里， SetState 是同步的？
...
<Input value={formData1.name} size="small" style={inputCssStyle} onChange={(value) => setFormData1("name",value.target.value)} />
            <Input value={formData1.password} size="small" style={inputCssStyle} onChange={(value) => setFormData1("password",value.target.value)} />
...
### 9.13.4 ant-design/pro-components 的学习
...
export const useConstructor = async (callBack = () => {}) => {
  const hasBeenCalled: any = useRef(false);
  if (hasBeenCalled.current) return;
  const next: any = await callBack();
...
useConstructor(async () => {
    const payload = await fetchUserList();
    if (payload) {
      setUsers([payload]);
    }
  });
...
### 9.13.14 git clone 加载不出来可能是 要在 git bash中拉取
