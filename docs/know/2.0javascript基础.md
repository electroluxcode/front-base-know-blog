...
注意const比较特殊
var 变量提升。只有声明没有赋值 。 并且声明优先级，函数大于变量。变量声明会被函数声明覆盖。越后的同名函数优先级越大
...
---------------------------------
上下文是一个执行环境。比如我们的window就是一个答的执行上下文呢，我们的函数就是一个小的执行上下文
...
var a = 10; //var 是 window 对象，会输出值。如果说是let 那么就不会输出值
function foo () {
   var a =6
  console.log(this.a) //这里是默认绑定 输出10 严格模式不输出
}
//我们在这里console.log() 就不是默认绑定
foo();  
console.log(a)
...
let new = function(param){
	let temp = {}
    temp.prototype = param.prototype
    this
}
```
...
### 2.1.12.symbol  | set | map | weakmap  | for in of
