...
https://gitee.com/Electrolux/front-css-package/blob/master/project%20and%20frame/three/12.ik/index_.html

```js
1.c4d里面新建一个物体
添加上材质什么的，然后右键转化成可编辑对象(c掉它)
2.点击 角色 - 关节工具 - ctrl + 左键进行关节 绑定 。建好后应该是有一个 跟对象 然后 递归的数组。命名为head - body - foot
3.c4d 中 选中刚才的骨骼和  物体进行 角色 - 绑定操作 
```
...
```js
1.首先 我们再 traverse中  找几个参数
// CCDIKHelper 的 第一个参数是下面的nodeBody.hands
...
```js
// 第二个参数 是 iks iks基本格式如下。这个number 类型 会找到上面  nodeBody.hands.skeleton.bones.我们呢要在这里面看 他的索引填上去
const iks = [
    {
...
this.scene.add( ccdikhelper ); //就可以了，但是这样子绑定会破坏原来的层级关系， 也就是说 c4d 绑定后就不需要继续绑定了
```
